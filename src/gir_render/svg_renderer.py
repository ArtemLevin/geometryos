...
SVG_COORDS: dict[str, tuple[int, int]] = {"A": (120, 40), "B": (40, 180), "C": (240, 180), "H": (120, 180), "M": (140, 180)}


def render_svg(scene: GirScene) -> str:
    parts = ['<svg xmlns="http://www.w3.org/2000/svg" width="280" height="220" viewBox="0 0 280 220">']
    ...
    for obj in scene.objects:
        if isinstance(obj, TriangleObject) and all(v in SVG_COORDS for v in obj.vertices):
            ...
            parts.append(f'<polygon points="{coords}" fill="none" stroke="black" stroke-width="2"/>')
            ...
            parts.append(f'<line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" stroke="#2563eb" stroke-width="2"/>')
    for obj in scene.objects:
